...
void UI::initializeGL(std::string assetsPath) {
	loadTextureFromFile(assetsPath + "cubetypes/grass.png", &m_grassTexture);
	loadTextureFromFile(assetsPath + "cubetypes/stone.png", &m_stoneTexture);
	loadTextureFromFile(assetsPath + "cubetypes/wood.png", &m_woodTexture);
	loadTextureFromFile(assetsPath + "cubetypes/glass.png", &m_glassTexture);
}

void UI::paintUI(int width, int height) {

	auto size{ImVec2(90, 400)};
    auto position{ImVec2(width - size.x, (height - size.y) / 2.0f)};
...
bool UI::loadTextureFromFile(std::string strFilename, GLuint* out_texture) {
